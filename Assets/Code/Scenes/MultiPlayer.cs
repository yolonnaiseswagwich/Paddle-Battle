...
public class MultiPlayer : BaseGame
{
    public Player mPlayer;
    public Multi mEnemy;
    private int SpawnChanger = 0;
    ...
    protected override void Update()
    {
        ...
        if (timer > 4.5f && NetworkP2P.IsReady())
        {
            for (int i = 0; i < 30; ++i)
            {
                ...
                if (Blocks[i] != null && !Blocks[i].Update())
                {
                    if (Blocks[i].GetMake())
                    {
                        ...
                        TempBall.SetDirection(new Vector2(SpawnChanger++ % 2 > 0 ? 0.7071067811865475f : -0.7071067811865475f, SpawnChanger++ % 2 > 0 ? 0.7071067811865475f : -0.7071067811865475f));
                        Balls.Add(TempBall);
                    }
                    Blocks[i] = null;
                }
            ...
            mPlayer.Update();
            if (!NetworkP2P.Update(ref mPlayer, ref mEnemy))
            {
                NetworkP2P.Shudown();
                SceneManager.LoadScene(1);
                return;
